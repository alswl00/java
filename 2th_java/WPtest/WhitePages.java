...
public class WhitePages {
	...
	/*public void deletewhitepages() {
		Scanner sc=new Scanner(System.in);
    	System.out.println("지울 사람의 전화번호를 입력하세요>>");
		String tele=sc.nextLine();
		for(WhitePagesObj item : whitePageList) {
			if(item.getTeleNum().equals(tele)) {
	...
	public void deletewhitepages() {
		Scanner sc=new Scanner(System.in);
		ArrayList<WhitePagesObj> list = new ArrayList<WhitePagesObj>();
		showList();
    	System.out.println("지울 사람의 이름을 입력하세요>>");
		String a = sc.nextLine();
		for(WhitePagesObj item : whitePageList) {
			if(item.getName().equals(a)) 
				list.add(item);
		}
		for(WhitePagesObj item : list) {
			if(list.size()<2) 
				whitePageList.remove(item);
			else {
				System.out.println("중복되는 이름이 있습니다. 제거할 사람의 번호를 입력해주세요.");
				int i = 0; int index;
				for(WhitePagesObj L_item : list) {
					i++;
					System.out.println(i+". "+L_item);
				}
				i=sc.nextInt();
				index=whitePageList.indexOf(list.get(i-1));
				whitePageList.remove(index);
			}
			break;
		}
	}
	...
	public static void main(String[] args) {
		...
		while(true) {
            ...
            System.out.println("3. Delete White Pages");
            System.out.println("4. End White Pages   ");
            System.out.println("=======================");
